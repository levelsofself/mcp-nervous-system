...
,{
    ...
    annotations: { title: 'Audit MCP Config (Governance Lint)', readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    description: 'Governance lint for MCP server configurations (free, local). Checks: plaintext secrets in env, unpinned packages, auto-install flags, broad filesystem scopes, shell wrappers, non-TLS remote transports, unpinned docker tags. Returns findings with severities and a 0-100 score.',
    inputSchema: { type: 'object', properties: { config: { type: 'object', description: 'The MCP config JSON as an object. Provide this OR config_json, not both.' }, config_json: { type: 'string', description: 'The MCP config JSON as a string; it will be parsed. Provide this OR config, not both.' } } }
  }
];
